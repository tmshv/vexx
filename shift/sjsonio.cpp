...
#define OBJECT_VALUE_CHAR_BYTEARRAY(key, valueString) COMMA_IF_REQUIRED NEWLINE_IF_REQUIRED TAB_ELEMENT _device->write("\"" key "\":\""); _device->write(valueString); _device->write("\"");
#define OBJECT_VALUE_BYTEARRAY_BYTEARRAY(key, valueString) COMMA_IF_REQUIRED NEWLINE_IF_REQUIRED TAB_ELEMENT _device->write("\""); _device->write(key); _device->write("\":\""); _device->write(valueString);  _device->write("\"");
...
SJSONSaver::SJSONSaver() : _autoWhitespace(true), _device(0), _root(0)
  {
...
void SJSONLoader::endNextChild()
  {
  xAssert(_current == ChildrenEnd || _current == AttributesEnd);
  }
