...
inline QString rendererVertexShader( QRectF rect )
    ...
    return "void main()"                                                                                                            \
        "{"                                                                                                                         \
        "gl_TexCoord[0] = ( ( 0.5 * ( 1.0 + gl_Vertex ) ) * vec4( " + QString::number(rect.width()) + ".0, " + QString::number(rect.height()) + ".0, 0.0, 0.0 ) ) + vec4( " + QString::number(rect.left()) + ", " + QString::number(rect.top()) + ", 0.0, 0.0 );"  \
...
void nodeRenderer::changed( APropertyDevice *, AProperty *data )
    ...
    if( data )
        ...
        if( data->attributes().contains("Attribute") )
            ...
            {
            if( data->name() == "nodeEntity/source" || data->attributes().contains("Attribute") )
                {
                _rebuild = TRUE;
                if( _settingTime == FALSE )
                    {
