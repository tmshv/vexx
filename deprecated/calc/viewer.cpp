...
void viewer::mousePressEvent( QMouseEvent *event )
    {
    // if theres no picker mode
    if( _activePickerMode == None )
        {
        // search for a colliding colour sampler
        bool found( false );
        ...
        if( found == false )
            {
            // if the mouse is dragging the splitter
            if( event->pos().x() > (width() - 20) && !_isHorizontallySplit )
                ...
                // try to find a node that can use the mouse event
                ...
                foreach( AEntity *sel, app()->selection() )
                    ...
                    if( node )
                        ...
                        if( node->canUse( &ev ) )
                            {
                            // node now has mouse, send it move and release events
                            _activeNode = node;
                            _activeManipulator = -1;
                            }
                        }
                    ...
                    foreach( AEntity *sel, app()->selection() )
                        ...
                        if( node )
                            ...
                            for( int y=0; y<node->manipulators().size(); y++ )
                                {
                                // see if a manipulator wants the mouse events
...
void viewer::mouseMoveEvent( QMouseEvent *event )
    {
    // try to forward the mouse event to where its needed
    QPointF nowCoord( _mouseTransform.map( event->posF() ) );
    if( _activePickerMode != None )
        {
        // try to update picker mode position
        ...
        if( _splitPosChanging && !_isHorizontallySplit )
            ...
            if( _activeNode != 0 ) // we are manipulating
                {
                // update manipulator if manipulating
...
void viewer::mouseReleaseEvent( QMouseEvent *event )
    ...
    if( _activePickerMode != None )
        ...
        if( _activePickerMode == Picking )
            ...
            {
            // delete the picker if it was clicked on
        ...
        {
        // no picker so try to inform node or manipulator
        if( _activeNode && _activeManipulator == -1 )
            {
            // give the node a use signal
            QMouseEvent ev( event->type(), nowCoord, nowCoord, event->button(), event->buttons(), event->modifiers() );
            _activeNode->use( &ev );
            }
        if( _isSampling )
            {
            // emit colour sampling
            _isSampling = false;
            QPointF nowCoord( _mouseTransform.map( event->posF() ) );
            emit colourSampleChanged( _renderer.sampleColourAt( (nowCoord-QPointF(0.5,0.5)).toPoint() ) );
            }

        // initiate manipulators and reset for next mouse event
...
void viewer::initiateManipulators()
    {
    // dont initiate if there is an active node
    if( !_activeNode )
        {
        // find scale paramaters to reset views
        ...
        foreach( AEntity *sel, app()->selection() )
            ...
            if( node )
                ...
                // for each manipulator in each selected node, set view scale and set up manipulators
...
void viewer::translateView( QPointF trans )
    {
    // translate view using current view transform
    QTransform tX;
...
void viewer::setDisplayTime( int t )
    {
    // render view depending on cache
    _frame = t;
